...
def cleanPlate(plate): #Method for finding the coordinated for a potential license plate
...
#Checking if the extracted contours are those of a rectangle and label it as such
		max_cnt = contours[max_index]
		max_cntArea = areas[max_index]
		x,y,w,h = cv2.boundingRect(max_cnt)

#If contours could not be extracted as a rectangle, discard
		if not check_ratio(max_cntArea,w,h):
			return plate,None

		cleaned_final = thresh[y:y+h, x:x+w]
		return cleaned_final,[x,y,w,h]
    #We only need to consider those rectangular fitgures, we can drop the rest for this consideration
    #cleaned_final tells us what to keep
...
def extract_contours(threshold_img):
  ...
  #Extracts the contours from the threshold image we created earlier in preprocess function
  #Retreives the shape of pixels we want, in this case we are looking for a rectangle
	element = cv2.getStructuringElement(shape=cv2.MORPH_RECT, ksize=(17, 3))
	morph_img_threshold = threshold_img.copy()
    
  #MorphologyEx function removes excess noise from a shape such as whitespace openings and fly pixels
...
def cleanAndRead(img, contours, filename):
  for i, cnt in enumerate(contours):
    min_rect = cv2.minAreaRect(cnt) # Draws rectangles using the contours and considers their rotation
